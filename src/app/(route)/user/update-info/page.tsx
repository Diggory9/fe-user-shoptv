...
export default function CheckOutPage() {
    ...
    useEffect(() => {
        ...
        const fetchInfo = async () => {
            try {
                const res = await ApiAuth.authGetInfo({ userId: auth.data?.id || '' });
                setInfo(res.data);
                form.setFieldsValue({
                    ...res.data,
    ...
    const handleFinish = async (values: any) => {
        ...
        console.log(body)
        ApiAuth.authUpdateInfo(body).then((result) => {
            alert('Thông tin đã được cập nhật thành công. Bạn cần đăng xuất để đồng bộ dữ liệu.');
            signOut();
            dispatch(removeAuth());
            dispatch(resetCart());
            ...
            >
                <div className="flex flex-col md:flex-row mx-10 my-5 space-y-5 md:space-y-0 md:space-x-5">
                    <div className="w-full font-serif">
                        <h1 className="text-3xl font-semibold mb-4">Cập nhật thông tin</h1>
                        <div className="flex space-x-5">
                            <Form.Item label="Họ và tên" name="fullName" className="w-1/2">
                        ...
                        <div className="flex space-x-5">
                            ...
                            <Form.Item label="Tên hiển thị" name="displayName" className="w-1/2">
                                <Input placeholder="Nhập tên hiển thị" />
                            </Form.Item>
                        </div>
                        <div className="flex space-x-5">
                            <Form.Item label="Tỉnh/Thành Phố" name="province" className="w-1/2">
                                <Select
                            ...
                            <Form.Item label="Quận/Huyện" name="district" className="w-1/2">
                                <Select
                        ...
                        <div className="flex space-x-5">
                            <Form.Item label="Phường/Xã" name="ward" className="w-1/2">
                                <Select placeholder="Phường/Xã" options={wardOptions} />
                            </Form.Item>
                            <Form.Item label="Địa chỉ" name="address" className="w-1/2">
