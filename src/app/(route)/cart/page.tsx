...
const CartPage = () => {
    ...
    return (
        <div className="container mx-auto px-4 py-8 bg-white">
            <Badge offset={[20, 0]} count={cart !== null && cart?.data?.length! > 0 ? cart.data?.length : 0} style={{ display: cart !== null && cart.data?.length! > 0 ? 'block' : 'none' }}>
                        ...
                        {cart?.data?.map((item) => (
                            ...
                            >
                                <div
                                    ...
                                    className="flex mx-10 my-15 py-4"
                                >
                                    <img
                                        src={item?.image?.url}
                                        alt={item?.name}
                                    ...
                                    <div>
                                        <div className="flex-1 m-5">
                                            ...
                                            <div className="flex items-center mt-5">
                                                ...
                                                <p className="rounded-full border-spacing-1 border-gray-600 w-6 h-6 mx-3" style={{
                                                    backgroundColor:
                                                        item.color?.colorCode || "transparent",
                                                }}></p>
                                            </div>
                                                    ...
                                                    <div className="mt-3">
                                                        <>
                                                            <p className="text-lg text-red-700">
                                                                {numberFormatLocationVietNam(handlePriceBeforeDiscount({ price: item.price || 0, typeDiscout: item.discount?.type || null, valueDiscount: item.discount?.value || null })!)}
                                                            </p>
                                                            <p className="text-base line-through text-gray-700 mt-1">
                                                                {numberFormatLocationVietNam(item?.price || 0)}
                                                            </p>
                                                        </>
                                                    </div>
